...
| `notify-hook/` | integration hook | automatic | Generic external notification bridge for pi lifecycle events and temporary user-attention states. Adapters self-register (Kitty, Herdr) so a missing adapter cannot prevent pi from starting. |
...
- `notify-hook/adapters/` holds platform adapters that self-register. Keep new backends there and list them in both `notify-hook/package.json` and the root `pi.extensions` so local discovery and package installs stay aligned.
