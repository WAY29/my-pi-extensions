...
| `better-edit/` | tool wrapper / edit advice | `/better-edit`, `edit` | Wraps Pi's built-in `edit` tool without changing the underlying edit engine. Adds looser argument compatibility, AI-friendly error advice, duplicate-match line hints, and a persistent on/off toggle for the enhanced wrapper. |
...
| `orca-agent-status.ts` | integration hook | automatic | Orca-managed status bridge that posts pi lifecycle snapshots to the local Orca endpoint. |
| `orca-prefill.ts` | integration hook | automatic | Orca-managed prefill helper for Orca-connected pi sessions. |
| `orca-titlebar-spinner.ts` | integration hook | automatic | Orca-managed titlebar spinner while the agent is working. |
