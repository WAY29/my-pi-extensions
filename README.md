...
| `codex-style-apply-patch/` | tool renderer / Codex-style patch tool | `/codex-style-apply-patch`, `apply_patch` | Adds a Codex-style `apply_patch` tool for GPT/Codex models, disables built-in `edit`/`write` while active, and renders patch previews as compact diff views using Pi's built-in `renderDiff` path. |
| `pi-codex-profile/` | provider header patch | automatic | Injects a Codex Desktop `User-Agent` on GPT model provider requests via `before_provider_headers`, so ChatGPT/Codex-compatible routes keep working after the ModelRuntime auth rewrite. |
| `better-grep/` | tool wrapper / grep guidance | `grep` | Wraps Pi's built-in `grep` tool with stronger literal-search guidance and automatically retries invalid regex patterns with `literal=true` so exact code searches like `AuthSpecialUser(` do not fail. |
...
| `builtin-tools.ts` | tool toggle command | `/builtin-tools` | Globally manages whether Pi exposes the built-in `grep`, `find`, and `ls` tools to the model. Supports group and per-tool toggles, argument completion, quiet global persistence, and current-vs-preference status output. |
...
| `notify-hook/` | integration hook | automatic | Generic external notification bridge for pi lifecycle events and temporary user-attention states. Adapters subscribe through pi's shared event bus (Kitty, Herdr), so a missing adapter cannot prevent pi from starting. |
| `notify-hook/attention.ts` | helper module | automatic | Shared helper for emitting temporary “awaiting user attention” lifecycle signals to notify-hook-aware extensions such as `notify-hook/`, `permission-gate.ts`, and `pi-sandbox/`. |
| `working-status.ts` | working indicator/UI status | automatic | Replaces pi's streaming `Working...` text with action-aware labels and a live elapsed timer. Also paints folded thinking as `thoughts · Xs`. Keeps showing the most recent tool action while the model continues, and leaves a dim `Finished working in ...` status after `agent_end` until the next run. |
...
| `herdr-agent-state.ts` | integration hook | automatic | Herdr-managed pane integration. Only activates inside herdr-managed panes. |
| `superset-hooks.ts` | integration hook | automatic | Emits Claude-Code-compatible lifecycle hooks to Superset's `notify.sh` for working/completion indicators. |
| `pi-glance/` | UI/input surface | `/glance` | Replaces the default input area with a rounded multiline editor and inline status glance for model, context, tokens, cost, Git, title, and plan state. Theme default can follow the host pi theme; the settings pane can also configure workspace auto-model rules, including `model[:thinking]` AutoModel rules, and toggle `permission-gate.ts` / `pi-sandbox/` when those extensions are installed. |
...
- `tool-call-summary.ts` to group file-tool calls and control noisy command/search output.
...
- `notify-hook/adapters/` holds platform adapters that subscribe to the shared notify lifecycle event. Keep new backends there and list them in both `notify-hook/package.json` and the root `pi.extensions` so local discovery and package installs stay aligned.
