...
test("accepts max thinkingLevel", () => {
	const config = mergeTrioConfig(TEST_CONFIG, { planner: { thinkingLevel: "max" } }, "test config");
	assert.equal(config.planner.thinkingLevel, "max");
});
