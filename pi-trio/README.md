...
Roles accept optional `thinkingLevel` (`off` | `minimal` | `low` | `medium` | `high` | `xhigh` | `max`) and `systemPrompt` fields. The optional top-level `maxReviewRounds` field sets a review-round cap. A standalone configuration includes a provider and model for all three roles. A project-local `.pi/trio.json` can override fields from a complete global configuration.
