...
const DEFAULT_MAX_REPO_SIZE_MB = 350;
const DEFAULT_CLONE_TIMEOUT_SECONDS = 30;
...
async function cloneRepo(owner: string, repo: string, ref: string | undefined, signal?: AbortSignal): Promise<string | null> {
  ...
  const timeoutMs = DEFAULT_CLONE_TIMEOUT_SECONDS * 1000;
  ...
  if (!forceClone) {
    ...
    if (sizeKB !== null) {
      ...
      if (sizeMB > DEFAULT_MAX_REPO_SIZE_MB) {
        const sizeNote =
          `Note: Repository is ${Math.round(sizeMB)}MB (threshold: ${DEFAULT_MAX_REPO_SIZE_MB}MB). ` +
