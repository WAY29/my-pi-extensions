...
| `pi-codex-profile/` | provider header 补丁 | 自动 | 通过 `before_provider_headers` 给 GPT 模型请求注入 Codex Desktop `User-Agent`，适配 ModelRuntime 鉴权改造后的主请求路径。 |
...
| `better-grep/` | 工具包装 / grep 引导增强 | `grep` | 包装 Pi 内置 `grep` 工具，强化字面量搜索提示，并在无效 regex 导致失败时自动用 `literal=true` 重试，避免 `AuthSpecialUser(` 这类精确代码搜索失败。 |
